...
export class LectureDetails implements OnInit, OnDestroy {
  ...
  activeCaptureToken: string | null = null;
  ...
  openMobileCapture() {
    ...
    this.captureService.startCaptureSession(sessionPayload).subscribe({
      next: (res) => {
        ...
        if (this.activeCaptureToken && this.activeCaptureToken !== res.token) {
          this.captureService.endCaptureSession(this.activeCaptureToken).pipe(take(1), catchError(() => of(void 0))).subscribe();
        }
        this.activeCaptureToken = res.token;
        const apiBase = this.captureService.apiBaseUrl;
        const mobileUrl = `${window.location.origin}/mobile-capture/${res.token}?api=${encodeURIComponent(apiBase)}`;
        ...
        this.ws.onmessage = (event) => {
            ...
            const message = data.result?.message || 'Capture processed.';
            this.statusMsg = message;
            if (data.result?.success && this.sessionId) {
              this.refresh();
            } else {
              this.cdr.detectChanges();
            }
  ...
  closeQrModal() {
     ...
     /* if (this.ws) {
      ...
      if (this.activeCaptureToken) {
        this.captureService.endCaptureSession(this.activeCaptureToken)
        .pipe(take(1), catchError(() => of(void 0)))
        .subscribe();
        this.activeCaptureToken = null;
      }
