...
class FaceRecognizer:
    ...
    def extract_embedding(self, image_input) -> Optional[List[float]]:
        """Extract face embedding using DeepFace with fallback strategies."""
        detectors: List[str] = [settings.DETECTOR_BACKEND]
        for candidate in ["retinaface", "mtcnn"]:
            if candidate not in detectors:
                detectors.append(candidate)

        # Strict detection first
        for detector in detectors:
            logger.debug(f"Attempting embedding extraction with '{detector}' (strict)")
            try:
                ...
                if embedding_objs:
                    logger.debug(f"Embedding extracted with detector '{detector}'")
                    return embedding_objs[0]["embedding"]
            except ValueError as e:
                logger.info(f"Face detector '{detector}' failed: {e}")
            except Exception as e:
                logger.error(f"Detector '{detector}' errored: {e}")

        # Relaxed detection as last resort
        for detector in detectors:
            logger.debug(f"Attempting relaxed detection with '{detector}'")
            try:
                embedding_objs = DeepFace.represent(
                    img_path=image_input,
                    model_name=settings.MODEL_NAME,
                ...
                if embedding_objs:
                    logger.warning(
                        f"Embedding extracted with relaxed detection using '{detector}'"
                    )
                    return embedding_objs[0]["embedding"]
            except Exception as e:
                logger.debug(f"Relaxed detector '{detector}' failed: {e}")

        logger.warning("No face detected after trying all detectors")
        return None
    ...
    def recognize_face_from_students(self, uploaded_base64_image: str, students: List[Dict], session_id: str) -> Tuple[Optional[str], Optional[str], float]:
        ...
        for i, student in enumerate(students, 1):
            ...
            if not student_image:
                logger.debug(f"[{i}/{len(students)}] Skipping {student_name}: missing image data")
                continue
            
            try:
                # Get or extract embedding (with caching)
                logger.debug(f"[{i}/{len(students)}] Processing {student_name} ({student_id})")
